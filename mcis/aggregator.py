...
AGG_SPEC: dict[str, tuple[str, str | Any]] = {
    ...
    "military_fraction": ("vessel_category", lambda x: (x == "military_para").mean()),
...
def compute_aggregation(df: pd.DataFrame, config: dict, bucket_col: str = "time_bucket") -> pd.DataFrame:
    ...
    if panel["time_bucket"].dt.tz is not None:
        panel["time_bucket"] = panel["time_bucket"].dt.tz_localize(None)
