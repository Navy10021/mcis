...
class RollingZScoreDetector:
    ...
    def fit(self, X: pd.DataFrame) -> RollingZScoreDetector:
        ...
        if X.empty:
            raise ValueError("Cannot fit RollingZScoreDetector on empty data")
    ...
    def predict(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if X.empty:
            return pd.DataFrame(index=X.index, columns=X.columns).fillna(0.0)
...
class EWMADetector:
    ...
    def fit(self, X: pd.DataFrame) -> EWMADetector:
        ...
        if X.empty:
            raise ValueError("Cannot fit EWMADetector on empty data")
